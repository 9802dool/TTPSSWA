...
export const metadata: Metadata = {
  title: "Noel Chase Hotel and Conference Centre | TTPSSWA",
  description:
    "Request accommodation at Noel Chase Hotel and Conference Centre — check-in and check-out — TTPSSWA.",
};

export default function NoelChaseHotelPage() {
  return (
    <>
      <SiteHeader />
      <main className="pt-[var(--site-header-stack)]">
          ...
          <div className="relative mx-auto grid max-w-7xl gap-10 px-4 py-20 sm:px-6 lg:grid-cols-[1fr_auto] lg:items-end lg:px-8 lg:py-28">
            <div>
              ...
              <p className="mt-5 max-w-lg text-[15px] leading-relaxed text-slate-300">
                8 rooms across three categories. Choose your dates, pick rooms, and submit a request — our coordinator
                confirms rates and availability.
              </p>
              <div className="mt-8 flex flex-wrap gap-6 text-sm text-white/70">
                <span className="flex items-center gap-2">
                  <span className="text-lg">🏨</span> 8 rooms
                </span>
                <span className="flex items-center gap-2">
                  <span className="text-lg">👤</span> Up to 10 guests
                </span>
                <span className="flex items-center gap-2">
                  <span className="text-lg">📧</span> Email confirmation
                </span>
              </div>
            </div>
            <div className="hidden rounded-2xl border border-white/10 bg-white/5 px-6 py-5 backdrop-blur-sm lg:block">
              <p className="text-xs font-semibold uppercase tracking-widest text-white/50">Room types</p>
              <ul className="mt-3 space-y-2 text-sm text-white/80">
                <li>2 &times; Presidential suite</li>
                <li>2 &times; Full bed room</li>
                <li>4 &times; Double bed room</li>
              </ul>
              <p className="mt-4 text-xs text-white/40">Rooms adjustable on request.</p>
            </div>
          </div>
        </section>
        ...
        <section className="bg-surface py-12 dark:bg-surface lg:py-16">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <h2 className="text-2xl font-bold tracking-tight text-ink sm:text-3xl">Our Location</h2>
            <p className="mt-2 text-[15px] text-muted">Find us on the map below.</p>
            <div className="mt-6 overflow-hidden rounded-xl border border-line shadow-corp">
              <iframe
                title="Noel Chase Hotel and Conference Centre location"
